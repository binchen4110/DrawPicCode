...
# 设置marker
marker_edge = 'white'
marker_memes = "#D42121"        #'#6BAED6'  ###FF4040 #F9CBC8 #F6563D
marker_twitter = '#376795'  #ED392B  D42121 E12D26
marker_weibo = '#6BAED6' #1D7EB7  CB7968  C8171C #AB1016 #B91419
marker_android = '#7E0610' ##085FA3   #67000D   980C13 #7E0610

# 设置x 和 y
x_ticks = np.array([0.001, 0.01, 0.1, 1.0])
y_ticks_hits = np.array([61.76, 61.38396667, 56.77086667, 56.0268])
y_ticks_map = np.array([20.3, 20.46056667, 15.94696667, 16.29916667])
...
# 设置两个折线
ax[0].plot(y_ticks_hits, marker='s', color=marker_weibo, markeredgecolor=marker_edge, markersize=12, label='Hits@100')
ax[1].plot(y_ticks_map, marker='h', color=marker_memes, markeredgecolor=marker_edge, markersize=14, label='Map@100')
...
# 设置两个图y的范围
ax[0].set_ylim(55, 62)  # most of the data
ax[1].set_ylim(15, 21)  # most of the data

# 设置y的刻度
ax[0].set_yticks([55, 58, 61])
ax[1].set_yticks([15, 18, 21])
...
plt.savefig("../pics/chris_lambda.pdf", dpi=1000, bbox_inches='tight')
plt.show()